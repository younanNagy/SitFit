...
def main():
    ...
    blinking_time_threshold = 20    #time to check the number of blinks
    ...
    while True:
        ...
        if(window.state == "calibrate"):
            ...
            if(window.position_fit_enable):
                ...
                if int(end_time_for_position_fit-start_time_for_position_fit)>5 and notification_type!="none" and window.position_fit_enable :

                    Warning("Uncomfortable Position Detected" ,notification_type)
                    start_time_for_position_fit = time.time()
            # print(window.blink_enable)        
            #blinking part
            ...
            if flag_initialized == True:
                ...
                if int(current_time) >= blinking_time_threshold:   
                    if TOTAL < 5:  #check if the min number of blinks is 15
                ...
                ########################################################################################
                if window.timer_enable:

                    end_time_for_blink=time.time()
                    if ear!=0 and not(lock):
                        start_time_for_blink=time.time()
                        lock=1
                    
                    # if ear != 0:
                    #     lock=0
                    if int(end_time_for_blink-start_time_for_blink)>5 and ear!=0 and window.timer_enable:
                        Warning("Time exceeded","You've been sitting for an hour")
                        start_time_for_blink = time.time()
                        lock=0
                ##############################################################################################     
